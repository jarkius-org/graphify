...
For user-facing status lines, keep the voice concise, constructive, and lightly upbeat. Preserve clarity over hype.
...
**MANDATORY: You MUST use Codex native subagents here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not dispatch subagents you are doing this wrong.**
...
**Step B2 - Dispatch ALL subagents in a single message (Codex v8/native subagents)**

> **Codex platform:** Uses `spawn_agent` + `wait_agent` + `close_agent` instead of the Agent tool.
> Prefer a standard implementation role such as `executor` for these extraction subtasks; do not use `worker` outside explicit team/swarm runtimes.
...
Call `spawn_agent` once per chunk — ALL in the same response so they run in parallel. Store each returned agent id with its chunk number. Build the message by wrapping the extraction prompt below in task-delegation framing:

```
agent = spawn_agent(agent_type="executor", message="Your task is to perform the following. Follow the instructions below exactly.\n\n<agent-instructions>\n[extraction prompt below, with FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, DEEP_MODE substituted]\n</agent-instructions>\n\nExecute this now. Output ONLY the structured JSON response.")
```

After all agents are dispatched, collect results sequentially:
```
result = wait_agent(targets=[agent.id]); close_agent(target=agent.id)   # repeat per agent id
```

Parse each result as JSON, then write it to `graphify-out/.graphify_chunk_NN.json` where `NN` is the chunk number. Accumulate nodes/edges/hyperedges across all results and write to `graphify-out/.graphify_semantic_new.json`.
...
- If the subagent returned valid JSON with `nodes` and `edges`, write it to `graphify-out/.graphify_chunk_NN.json`, include it, and save it to cache
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or returned invalid JSON, stop and tell the user to re-run after confirming Codex native subagents are enabled.

Merge all chunk files into `graphify-out/.graphify_semantic_new.json`. Codex native subagent results may not expose token usage; keep the chunk-provided token counts when present, otherwise leave them at zero. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
